...
org-mode's own =org-activate= functions to do the heavy lifting. We then
branch depending on whether we're entering or exiting the mode:

org-highlight-links - default value is
...
This looks like a bug in =org-unfontify-region=.

Text may still have the =fontified= property, but that belongs to
=font-lock-mode= so is not for us to remove.
